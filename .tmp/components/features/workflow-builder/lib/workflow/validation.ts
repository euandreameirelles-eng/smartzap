...
export function canConnectHandle(params: {
	...
	// Debug log
	console.log('[canConnectHandle]', { nodeId, handleId, type, nodeType: node?.type, edgesCount: edges.length });
	...
	// Para source handles, verificar se já existe conexão
	if (type === "source") {
		const existingEdge = edges.find(
			(e) => e.source === nodeId && e.sourceHandle === handleId,
		);
		console.log('[canConnectHandle] source check:', { existingEdge: !!existingEdge, handleId });
		if (existingEdge) {
			return false;
		}
	}
...
/**
 * Validates graph structure requirements: exactly one start node.
 */
