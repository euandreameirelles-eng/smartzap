import type { Node } from "@xyflow/react";
import { z } from "zod";
import type {
	NodeSharedDefinition,
	ValidationContext,
	ValidationError,
} from "@/components/features/workflow-builder/types/workflow";

export const documentNodeDataSchema = z.object({
	status: z.enum(["processing", "error", "success", "idle"]).optional(),
	label: z.string().optional(),
	documentUrl: z.string().optional(),
	// Filename - max 240 chars (WhatsApp limit)
	filename: z.string().max(240, "Máximo 240 caracteres").optional(),
	// Caption - max 1024 chars (WhatsApp limit for media captions)
	caption: z.string().max(1024, "Máximo 1024 caracteres").optional(),
	validationErrors: z.array(z.any()).optional(),
});

export type DocumentNodeData = z.infer<typeof documentNodeDataSchema>;
export type DocumentNode = Node<DocumentNodeData, "document">;

function validateDocumentNode(
	node: DocumentNode,
	context: ValidationContext,
): ValidationError[] {
	const errors: ValidationError[] = [];
	const { edges } = context;

	const incomingEdges = edges.filter((e) => e.target === node.id);
	if (incomingEdges.length === 0) {
		errors.push({
			type: "invalid-node-config",
			severity: "warning",
			message: "Nó de documento não tem conexão de entrada",
			node: { id: node.id },
		});
	}

	if (!node.data.documentUrl?.trim()) {
		errors.push({
			type: "invalid-node-config",
			severity: "error",
			message: "URL do documento é obrigatória",
			node: { id: node.id },
		});
	}

	// Filename max 240 chars
	if (node.data.filename && node.data.filename.length > 240) {
		errors.push({
			type: "invalid-node-config",
			severity: "error",
			message: "Nome do arquivo excede 240 caracteres",
			node: { id: node.id },
		});
	}

	// Caption max 1024 chars
	if (node.data.caption && node.data.caption.length > 1024) {
		errors.push({
			type: "invalid-node-config",
			severity: "error",
			message: "Legenda excede 1024 caracteres",
			node: { id: node.id },
		});
	}

	return errors;
}

export const documentSharedDefinition: NodeSharedDefinition<DocumentNode> = {
	type: "document",
	dataSchema: documentNodeDataSchema,
	validate: validateDocumentNode,
};
